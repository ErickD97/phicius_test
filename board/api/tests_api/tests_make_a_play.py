...
class MakeAPlayTests(APITestCase):
    """ Tests for 'board:make_movement' api view. """
    ...
    def setUpTestData(cls):
        ...
        cls.url = reverse("board:make_movement")
    ...
    def test_wrong_payload(self):
        ...
        success.extend([
            ...
            response.url == reverse("board:board_list"),
            board.positions_circle == self.board.positions_circle,
            board.positions_cross == self.board.positions_cross,
        ])
    ...
    def test_status_changes(self):
        ...
        success.extend([
            response.status_code == 302,
            response.url == reverse("board:board_play", kwargs={"pk": self.board.pk}),
            board.status == 3,
        ])
        ...
        self.client.logout()
        self.client.force_login(user=self.user2)
        self.board.positions_circle["A"], self.board.positions_circle["C"], self.board.positions_circle["B"] = [1, 2], [1, 2], [3]
        self.board.positions_cross["A"], self.board.positions_cross["B"], self.board.positions_cross["C"] = [3], [1], [3]
        self.board.status = 1
        self.board.save()
        ...
        self.client.logout()
        self.client.force_login(user=self.user1)
        self.board.positions_circle["A"], self.board.positions_circle["B"], self.board.positions_circle["C"] = [1, 2], [], []
        self.board.positions_cross["A"], self.board.positions_cross["B"], self.board.positions_cross["C"] = [3], [1], []
        self.board.status = 1
        self.board.save()
        data_dict.update({"position": "B_2"})
        response = self.client.post(self.url, data=data_dict)
