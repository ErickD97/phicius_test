...
class NewMoveStructure(BaseModel):
    ...
    def board_exists(cls, v, values, **kwargs):
        ...
        if not exists:
            raise ValidationError(MESSAGES["SYS003"].value.format(f"{Board._meta.verbose_name.title()}"))
        return v
    ...
    def valid_position(cls, v, values, **kwargs):
        ...
        if values.data.get("board_id", None):
            ...
            if int(row) in board.positions_circle[column] or int(row) in board.positions_cross[column]:
