    ...
    def test_get_request(self):
        ...
        success.extend(
            [
                ...
                all(item in form_choices for item in choices)
                and all(item in choices for item in form_choices),
            ]
        )
        self.assertTrue(all(success))
    ...
    def test_post_requests(self):
        ...
        success.extend(
            [
                ...
                Board.objects.filter(
                    player_circle=self.user1, player_cross=self.user2, status=1
                ).exists(),
            ]
        )
        ...
        success.extend(
            [
                response.status_code == 302,
                Board.objects.filter(
                    player_circle=self.user3, player_cross=self.user1, status=1
                ).exists(),
            ]
        )
