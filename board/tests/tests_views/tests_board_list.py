...
class BoardListTests(TestCase):
    ...
    def test_get_request(self):
        ...
        queryset = Board.objects.filter(
            Q(player_cross=self.user1) | Q(player_circle=self.user1)
        )
        success.extend(
            [
                response.status_code == 200,
                ...
                all([item in queryset for item in response.context_data["board_list"]])
                and all(
                    [item in response.context_data["board_list"] for item in queryset]
                ),
        ...
        self.assertQuerysetEqual(
            response.context_data["boards_as_circle"], self.user1.circle.all()
        )
        self.assertQuerysetEqual(
            response.context_data["boards_as_cross"], self.user1.cross.all()
        )
        ...
        success.extend(historic_record)
        self.assertQuerysetEqual(
            response.context_data["boards_as_circle"], self.user2.circle.all()
        )
        self.assertQuerysetEqual(
            response.context_data["boards_as_cross"], self.user2.cross.all()
        )
        ...
        success.extend(historic_record)
        self.assertQuerysetEqual(
            response.context_data["boards_as_circle"], self.user3.circle.all()
        )
        self.assertQuerysetEqual(
            response.context_data["boards_as_cross"], self.user3.cross.all()
        )
    ...
    def test_filter(self):
        ...
        response = self.client.get(url)
        queryset = Board.objects.filter(
            Q(player_cross=self.user1, status=2) | Q(player_circle=self.user1, status=3)
        )
        self.assertQuerysetEqual(queryset, response.context_data["board_list"])
        ...
        response = self.client.get(url)
        queryset = Board.objects.filter(
            Q(player_cross=self.user1, status=3) | Q(player_circle=self.user1, status=2)
        )
        self.assertQuerysetEqual(queryset, response.context_data["board_list"])
        ...
        response = self.client.get(url)
        queryset = Board.objects.filter(
            Q(player_cross=self.user1, status=4) | Q(player_circle=self.user1, status=4)
        )
        self.assertQuerysetEqual(queryset, response.context_data["board_list"])
